...
function createMakerUtilitiesName(card){
  ...
  for(let x in card){
    ...
    console.log('makerUtilitiesName: ', makerUtilitiesName)
    ...
    for (var i = 0; i <= makerUtilitiesName.createUtilities.length; i++) {
        if (document.querySelector(`.${makerUtilitiesName.createUtilities[i]}`)) {
            let breakpoints = '';
            ...
            let stylesss = `
                ...
                `;
                document.getElementsByTagName('style')[0].appendChild(document.createTextNode(stylesss));
        }
    }
}
